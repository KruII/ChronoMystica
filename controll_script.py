...
class Control:
        ...
        if any(self.keyboard.is_key_pressed(key) for key in enter_keys):
            if current_time - self.last_enter_press_time > self.key_repeat_delay:
                ...
                # Okno Ustawień Audio
                    ...
                    if self.screen_thread.options == 1:
                        threading.Thread(target=self.user_input_audio, args=("MASTER",)).start()
                    # Przycisk Opcji Audio Ustawiania_Music
                    elif self.screen_thread.options == 2:
                        threading.Thread(target=self.user_input_audio, args=("MUSIC",)).start()
                    # Przycisk Opcji Audio Ustawiania_Game
                    elif self.screen_thread.options == 3:
                        threading.Thread(target=self.user_input_audio, args=("GAME",)).start()
                    # Przycisk Opcji Audio Ustawiania_Inputs
                    elif self.screen_thread.options == 4:
                        threading.Thread(target=self.user_input_audio, args=("INPUTS",)).start()
                    # Przycisk Powrotu
                ...
                # Okno Graj
                    ...
                    if self.screen_thread.options == 1:
                        ...
                        self.onlineStatus = self.onlineServer.connect_with_serwer()
                        self.screen_thread.test = self.onlineStatus
                        pass
                    elif self.screen_thread.options == 3:
                        self.screen_opener(0,["START", "OPTIONS", "QUIT"],1, True)                          
    ...
    def user_input_audio(self, name):
        self.screen_thread.CheckingAudio = True
        ...
        self.settings_r.save_config(self.limitetFPS, self.SHOW_FPS, self.quality, self.advanced_control, self.audio_level)
        time.sleep(0.5)
        self.screen_thread.CheckingAudio = False
        ...
        while self.running:
            ...
            if deltaU >= 1:
                if self.screen_opt == 0:
                    self.user_input()
                elif not self.screen_thread.CheckingKeyboard and not self.screen_thread.CheckingAudio:
